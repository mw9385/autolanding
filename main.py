...
parser.add_argument('--max_step', default=80000, type=int)
...
# Load chunked data into buffer1
with open('./memory/data_1.pkl', 'rb') as f:
    buffer1 = load(f)
# Load chunked data into buffer2
with open('./memory/data_2.pkl', 'rb') as f:
    buffer2 = load(f)
# Add buffer2 to buffer1
replay_buffer.combine(buffer1)
replay_buffer.combine(buffer2)
# If you don't need buffer2 anymore, you can delete it to save memory:
del buffer1
del buffer2
# load replay memory from data buffer
print("Data has been loaded. Length:{}".format(replay_buffer.__len__()))
...
def main():    
    ...
    global_step = 0
    for step in tqdm(range(int(args.max_step))): 
        # step = step + len(replay_buffer)
